...
For each baseline this file states the exact source, the exact reported figure,
the cost model, and why it is or is not comparable, so a skeptical reader can
audit every comparison without re-deriving the literature. For the provenance
of *our* circuits — search method, untouched run archives, per-record
reproduction — see
...
- **2026-07-27.** This repository now also ships two 88-gate circuits. Neither
  ...
  - `circuits/mixcolumns_88gates_depth7.json` **matches** Jean's published
    ...
    **Jean has priority**: that paper was posted 2026-07-23, the circuit here
    ...
    never "a new record".
  - `circuits/mixcolumns_88gates_depth8.json` is **derived**: its seed chain
    passes through Jean's 88 (chain in Claim 4). It is not an independent
    construction, Jean is credited for the material it descends from, and it is
    in any case **dominated by the 88 @ depth 7 above** (same count, greater
    depth), so it improves no frontier point. Shipped as a verified distinct
    construction, not as a claim.
  - Sun–Yang–Li's 89 (ePrint 2025/1493) still states no depth. This project
    ...
    **measured depth 9** on its transcription; wherever a depth is needed for
    that point, it is that measurement, labelled as such, and not a figure from
    the paper.
  - Unchanged: 97 @ 3, 92 @ 4 and 89 @ 5 remain the smallest we are aware of at
    their depths, and all three remain on the depth–count Pareto frontier.
- **2026-07-23.** Two publications change the comparison landscape, one of
  ...
  - Sun, Yang, Li, *Revisit the Boyar-Peralta Algorithm to Solve the Shortest
    ...
    stated. It predates this repository. Our sweeps of 2026-07-10/15 — and the
    2026 comparison tables we corroborated against (HILL Table 13; Xu–Sun ToSC
    2026(2) Table 4, which still lists 91 as state of the art) — missed it.
    Statements previously made here that "the smallest published count is 91"
    were therefore wrong, and the earlier claim that our 89-gate circuits were
    "two fewer than the smallest published count" is withdrawn.
...
is the longest input-to-output gate path. The literature calls this **g-XOR**
or an **SLP** (straight-line program) implementation.

**The s-XOR (in-place) model is comparable.** An s-XOR program is a sequence of
in-place instructions `x[i] <- x[i] XOR x[j]` on 32 registers initialized with
...
[Non-comparable results](#non-comparable-results) below): multi-input XOR gates
(XOR3/XOR4), technology-mapped gate-equivalent (GE) area, and in-place quantum
CNOT circuits.
...
MixColumns has outputs of weight 7), so only the count at depth 3 can improve.

**Conclusion:** the published depth-3 record is 99;
...
- The circuits are nevertheless different. The method repository transcribed
  ...
  is in the
  [method repository](https://github.com/Joe-b-20/slp-plateau-search) and named
  in `bounds.json`.
...
different cost functions; none is claimed against, in either direction:

- **44 gates at depth 3** (Pehlivanoğlu–Demir, PeerJ CS 2024): 5 XOR2 + 7 XOR3
  + 32 XOR4 — a multi-input gate model.
...
- **105–107 CNOTs at depth 10** (Zhang et al., IEEE TC 2024 — 105, the record;
  Shi–Feng, ASIACRYPT 2024, ePrint [2024/381](https://eprint.iacr.org/2024/381)
  — 107; Xu–Sun, ToSC 2026(2) — 105–107): in-place quantum CNOT circuits, a
  different resource model. Xu–Sun's classical result is 105 XORs, above the
  records here.
...
failure mode: they corroborated a 91-XOR floor against every 2024–2026
comparison table, yet ePrint 2025/1493 (89 g-XOR, posted 2025-08) was missed by
those tables and by us alike. Treat every negative claim here as best-effort
with a date, not as proof of absence.
